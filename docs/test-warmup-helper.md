...
+ `-Mechanism Run`, so a **zero-parameter elevated run** installs the fix fleet-wide. Keep
`ArcInputFix.exe` (Paint-alias) as the fallback.

> **Ship the script from a PERSISTENT folder** (e.g. under `%ProgramFiles%` or a managed
> share synced locally). The installer copies the resolved alias into its own folder and
> points the `Run` value at that copy (see the multi-user gap fix below), so a
> temp/removable location would break the launcher.

#### Multi-user gap fix (pre-existing profiles)

`Add-AppxProvisionedPackage` (run by `-Scope AllUsers`) only **stages** the package
machine-wide and auto-registers it for **brand-new** profiles at first logon. **Pre-existing**
accounts are not auto-registered, so their per-user App Execution Alias
(`%LOCALAPPDATA%\Microsoft\WindowsApps\ArcInputFixLifted.exe`) never appears and a per-user
`%LOCALAPPDATA%` `Run` value would point at nothing (seen on the Dell: a second, pre-existing
account had no alias after a successful *"Provisioned package for all (future) users"* run).

The installer closes this with the **simplest workable approach**: after `Add-AppxPackage`
it **copies the resolved alias into its own folder** (`$PSScriptRoot\ArcInputFixLifted.exe`)
and points the `Run` value (HKLM for `AllUsers`, HKCU for `CurrentUser`) at **that fixed
copy** - one absolute path shared by every user, so the launcher resolves even for accounts
that never registered the package. `explorer.exe` still launches it at logon, preserving the
proven shell-launch context. The copy is guarded (verifies the alias exists and the copy is
non-empty) because WindowsApps aliases are reparse points and a naive copy could become a
