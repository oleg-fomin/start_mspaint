<#
.SYNOPSIS
    Dev/test signing for ArcInputFixWarmup.msix using a self-signed certificate
...
.DESCRIPTION
    ...
    4. Exports the public cert to <OutDir>\ArcInputFixWarmup.cer so the test
...
if (-not $cert) {
    ...
    $cert = New-SelfSignedCertificate `
        ...
        -FriendlyName 'ArcInputFixWarmup Dev Signing' `
...
$cerPath = Join-Path $OutDir 'ArcInputFixWarmup.cer'
...
Write-Host "       Import-Certificate -FilePath ArcInputFixWarmup.cer -CertStoreLocation Cert:\LocalMachine\TrustedPeople"
